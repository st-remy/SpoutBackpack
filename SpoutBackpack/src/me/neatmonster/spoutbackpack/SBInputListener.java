...
public class SBInputListener extends InputListener {
		...
		if (event.getKey() == getKeyInConfig("Backpack.Key", "B")) {
									...
									"Money: "
											+ String.format(plugin.Method.format(plugin.Method
													.getAccount(
															player.getName())
													.balance())))
									.setTextColor(new Color(1, 1, 1))
									.setX(plugin.widgetX).setY(plugin.widgetY);
							player.getMainScreen().attachWidget(widget);
							widget.setVisible(true).setDirty(true);
			...
			if (!plugin.openedInventoriesOthers.containsKey(event.getPlayer()
				...
				if (!plugin.openedInventories.containsKey(event.getPlayer()
						.getName())) {
					if ((plugin.permissionHandler != null || plugin.permissionsBukkitPlugin != null)
							&& !plugin.userHasPermission(event.getPlayer(),
									"backpack.workbench")) {
						return;
					}
	...
	private Keyboard getKeyInConfig(String configProperty, String configDefault) {
		plugin.config.load();
		String keyInConfig = plugin.config.getString(configProperty,
				configDefault);
		plugin.config.save();
